...
class ResGCN(torch.nn.Module):
    ...
    def augmentation(self, image, label, grads, aug, eta):
        ...
        # print("centroid", image_present)
        # print("centroid_shape", image_present.shape)
        ...
        while True:
            ...
            perturbed_images = image.unsqueeze(1) + torch.normal(mean=epsilon, std=epsilon*0.04)*sign_data_grad.unsqueeze(0)  # [batch, 50 , hidden_dim]

            # print("perturbed_images:", perturbed_images)
            # print("perturbed_images_shape", perturbed_images.shape)

            h_origin = self.predictor(image)     # [batch, num_classes]
            h_pred = self.predictor(perturbed_images)   # [batch, 50, num_classes]

            # print("h_origin:", h_origin)
            # print("h_origin_shape", h_origin.shape)
            # print("h_pred:", h_pred)
            # print("h_pred_shape", h_pred.shape)
            ...
            if label == None:
                ...
                cur_pred = h_pred[index_tuple]  # [batch, 50]
                # print("cur_pred", cur_pred)
                # print("cur_pred_shape", cur_pred.shape)
            else:
                arg_h_origin = torch.argmax(label, dim=-1)
                # print("h_pred", h_pred)
                # print("h_pred_shape", h_pred.shape)
                cur_pred = h_pred * label.unsqueeze(1)
                # print("cur_pred1", cur_pred)
                # print("cur_pred1_shape", cur_pred.shape)
                # print("label.unsqueeze(1)", label.unsqueeze(1))
                # print("label.unsqueeze(1)_shape", label.unsqueeze(1).shape)
                cur_pred, _ = torch.min(cur_pred, dim=-1)
                # print("cur_pred2", cur_pred)
                # print("cur_pred2_shape", cur_pred.shape)

            eq = arg_h_pred - arg_h_origin.unsqueeze(1) # [batch, 50]
            # print("eq", eq)
            # print("eq_shape", eq.shape)
            eq_mask = (eq==0) # [batch, 50]
            # print("eq_mask", eq_mask)
            # print("eq_mask_shape", eq_mask.shape)
            neq_mask = (eq_mask==False) # [batch, 50]
            # print("neq_mask", neq_mask)
            # print("neq_mask_shape", neq_mask.shape)
            row_check = torch.any(eq_mask, dim=-1)  # 判断每一个图数据是否有一个分类和origin（或label）增强相同的样本 [batch,]
            # print("row_check", row_check)
            # print("row_check_shape", row_check.shape)
            ...
            break
        # print("final", perturbed_image_final)
        # print("final_shape", perturbed_image_final.shape)

        # perturbed_image_final  [batch, hidden_dim]
        return perturbed_image_final

    def COSTA(self, x, label, grads, aug, eta):
        k = torch.tensor(int(x.shape[0]))
        p = (1 / torch.sqrt(k)) * torch.randn(k, x.shape[0]).cuda()
        x_perturbed = p @ x
        return x_perturbed


    def forward_BNConvReLU_cl(self, x, edge_index, batch, xg=None, aug=False, label=None, grads=None, eta=1.0):
        # fc = torch.nn.Linear(64, 4)
        x = self.conv1(x, edge_index)
        x = torch.relu(x)
        x = self.conv2(x, edge_index)
        x = torch.relu(x)
        ...
        if aug:
            ...
            #x = self.COSTA(x, label, grads, aug, eta)
